...
class LoginViewController : UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if ref.authData != nil {
            self.performSegueWithIdentifier("CONVERSATIONS", sender: ref.authData)
        }
    }
        ...
        ref.authUser(email, password: password,
            withCompletionBlock: { error, authData in
                if error != nil {
                   ...
                   self.performSegueWithIdentifier("CONVERSATIONS", sender: authData)
                }
        })
    }
    
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject!) {
        var conversationsVc = segue.destinationViewController as ConversationsTableViewController
